...
function doRectanglesOverlap(rect1, rect2) {
  const row = (rect1.left + rect1.width <= rect2.left || rect2.left + rect2.width <= rect1.left);
  const column = (rect1.top + rect1.height <= rect2.top
...
function findFirstSingleChar(str) {
  return str.split('').filter((el, ind, arr) => ((arr.indexOf(el) === arr.lastIndexOf(el)) ? el : false))[0];
}
...
function isCreditCardNumber(ccn) {
  const card = ccn.toString().split('').map((el, ind, arr) => {
    if ((arr.length - 1) % 2 === 0) {
      if (ind % 2 !== 0) {
  ...
  const res = card.reduce((total, el) => total + +el, 0);
...
function getDigitalRoot(num) {
  const res = num.toString().split('').reduce((total, el) => total + +el, 0);
...
function isBracketsBalanced(str) {
  ...
  str.split('').forEach((el) => {
    if (closeBracket.includes(el)) {
      if (stack[stack.length - 1] === openBracket[closeBracket.indexOf(el)]) {
        stack.pop();
      } else {
        stack.push(el);
      }
